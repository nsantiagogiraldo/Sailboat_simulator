...
class sailboat_environment(ts.train_test_scenarios):
    ...
    def save_data(self,data,control_action = 0):
        ...
        if self.learning:
            ...
            self.base.append_data([self.state+1,self.real_wind_angle,
    ...
    def control_inputs(self, data, max_rate, min_rate, real = False):
        ...
        if real:
            ...
            vec2=[self.waypoints[self.state+1][0],self.waypoints[self.state+1][1]]
            vec1=[data[1],data[2]]
            self.desired_heading = self.angle_vectors(vec2-vec1,np.array([1,0]))
    ...
    def dist_GPS(lat,lon,R=6378000):
        lon1 = np.radians(lon)
        lat1 = np.radians(lat)
    ...
    def true_wind(app_wind_angle,app_wind_speed,sailboat_speed):
